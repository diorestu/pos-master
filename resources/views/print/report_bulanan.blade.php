...
<body>
    ...
    <h6 class="text-reset mb-3">Bulan {{ $date }}</h6>
    <h5 class="mb-0">Rekapitulasi Pemasukan</h5>
    <table class="table table-sm table-bordered">
        ...
        <tbody>
            <tr>
                ...
                <td class="text-right">{{ number_format($awal->sum('saldo_awal')) }}</td>
            </tr>
            ...
            <tr>
                ...
                <td colspan="1" class="text-right" style="vertical-align: middle !important">
                    <h3>{{ number_format($payment->sum('total') + $awal->sum('saldo_awal')) }}</h3>
                </td>
            </tr>
        </tbody>
    </table>
    <h5 class="mt-3 mb-1">Rekapitulasi Pengeluaran</h5>
    <table class="table table-sm table-bordered">
        <thead style="background-color: #ced4da; color: #333;">
            <tr>
                <th class="text-center" width="40">No.</th>
        ...
        <tbody>
            @forelse ($out as $item)
                <tr>
                    ...
                    <td class="text-center">{{ tglIndo($item->created_at) }}</td>
    ...
    <br />
    @php
        $cash_in = $payment->sum('total') + $awal->sum('saldo_awal');
        $cash_out = $out->sum('jumlah');
        $saldo_akhir = $cash_in - $cash_out;
    @endphp
    <table class="table table-sm table-bordered">
        <tbody style="background-color: #ced4da; color: #333;">
            <tr>
                <td colspan="3" class="text-right" style="vertical-align: middle !important">
                    <h3 style="">SALDO AKHIR&nbsp;&nbsp;&nbsp;</h3>
                </td>
                <td colspan="1" class="text-right" style="vertical-align: middle !important" width="100">
                    <h3>{{ number_format($saldo_akhir) }}</h3>
                </td>
            </tr>
        </tbody>
    </table>
